...
자바의 `float` 형과 `double` 형은 https://standards.ieee.org/findstds/standard/754-1985.html[IEEE 754-1985] 에 정의된 단정도(single-precision, 32-bit)와 배정도(double-precision, 64-bit)를 표현한다. 각각의 형은 한 개의 부호(sign) 비트, 지수(exponent)를 표현하는 부분, 그리고 가수(fraction)를 표현하는 부분으로 구성된다.
...
(거의 모든) 컴퓨터들은 자료를 저장할 때, `octet` 으로 불리는, 여덟개의 비트로 구성된 최소 단위의 숫자들로 나누어 저장한다. `a` 라는 문자는 (`US-ASCII` 로 인코딩 되었을 때) `01100001` 로 저장되고 `홍` 이라는 문자는 (`UTF-8` 로 인코딩 되었을 때) `111011011 00110011 0001101` 로 저장된다. 아래 표에 여덟개의 비트로 표현할 수 있는 몇몇 값들을 10진법, 8진법, 2진법, 그리고 자바의 `byte` 값으로 표현하였다.
...
거의 모든 문자셋의 기본이 되는 `US-ASCII` 문자셋에는 줄바뀜(CR, LF), 탭(TAB) 등의 제어 문자들과 보통의 자판에서 찾아볼 수 있는 문자들을 포함하여 `128`(`2^7^`)개의 코드가 정의되어 있다. 때문에 여덟개의 비트로 표현할 수 있는 `256`(`2^8^`)개의 수에 고유의 문자를 할당하는 것이 불가능하다. 이를 해결하는 방법 중에 하나가 `base16`(`hex`) 로 표현하는 것이다. `base16` 표기는 여덟 개의 비트를 네 개씩 두 그룹으로 나눈 후에 각 그룹의 값을 미리 정의된 문자로 변환한다.
...
그리고 (네 개의 비트로 구성되는) 각 그룹의 값(`0` ~ `15`)을 아래 표에 정의된 열 여섯개의 문자들 중 하나로 변환한다. (한 개의 숫자가 두개의 문자로 변환된다.)
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=58..63]
----
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=73..79]
----
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=42..49]
----
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=58..63]
----
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=73..79]
----
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=87..92]
----
...
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=62..69]
----
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableFilterChannel.java[lines=49..53]
----
...
.WritableHexChannel.java
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=33..34]

    // 인코더 객체도 Supplier 를 통해 게으르게 생성한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=43..47]

    // 게으르게 생성된 encoder 를 반환한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=54..59]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=77..87]
----
...
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=68..76]
----
...
.ReadableHexChannel.java
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=33..34]

    // 인코더 객체도 Supplier 를 통해 게으르게 생성한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=45..50]

    // 게으르게 생성된 encoder 를 반환한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=57..62]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=92..102]
----
...
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=75..91]
----
