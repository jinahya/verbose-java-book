...
보는 바와 같이 `011` 에서 `1` 을 더했을때 `100 (+4)` 이 되지만 첫번째 비트가 `1` 이므로 음수(`-4`)로 처리된다. 최대값은 첫번째 비트가 `0` 이고 나머지 모든 비트가 `1` 인 값이고 최소값은 첫번째 비트가 `1` 이고 나머지 비트가 모두 `0` 인 값이다. 이를 수식으로 표현하면 최대값은 `+2^n-1^-1` 이고 최소값은 `-2^n-1^` 이다.
...
자바의 `float` 형과 `double` 형은 https://standards.ieee.org/findstds/standard/754-1985.html[IEEE 754-1985] 에 정의된 단정도(single-precision, 32-bit)와 배정도(double-precision, 64-bit)를 표현한다. 각각의 형은 한 개의 sign bit과 지수를 표현하는 부분(exponent) 그리고 가수를 표현하는 부분(fraction)으로 구성된다.
...
이를 해결하기 위해 exponent를 구성하는 비트들이 모두 `0` 일 경우 비정규화된(denormalized) 값으로 구분하며 `I` 의 값으로 `0.0` 을 사용하고 `E = 1 - bias` 이다.

===== &#177;Zero

exponent 부분의 비트들이 모두 `0` 이고 fraction 부분의 비트들도 모두 `0` 이면 `0.0` 을 나타낸다. 흥미롭게도 sign bit 에 따라서 `+0.0` 과 `-0.0` 을 따로 표현할 수 있다. 다음의 값을 실수로 계산해 보면
