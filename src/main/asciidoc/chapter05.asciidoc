...
{apidocs-javase}/api/java/io/FilterInputStream.html#read--[`FilterInputStream#read()`] 메서드는 `in` 객체의 `read()` 메서드 값을 반환한다. `HexInputStream` 클래스는 아래와 같이 바이트 두 개를 읽어서 하나로 변환한 후 반환하도록 재정의되었다.
