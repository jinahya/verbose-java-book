...
보는 바와 같이 `011 (+3)` 에서 `1` 을 더했을때 `100 (+4)` 이 되지만 첫번째 비트가 `1` 이므로 음수(`-4`)로 처리된다.
...
다음은 `&#177;.0f` 과 {apidocs-javase}/java/lang/Float.html[`java.lang.Float`] 클래스에 정의된 상수들을 2진법으로 출력한 내용이다. (보는 바와 같이 `java.lang.Float.NaN` 은 quiet NaN 이다.)
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=42..49]
----
