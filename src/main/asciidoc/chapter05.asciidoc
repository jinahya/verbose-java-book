...
자바의 `float` 형과 `double` 형은 https://standards.ieee.org/findstds/standard/754-1985.html[IEEE 754-1985] 에 정의된 단정도(single-precision, 32-bit)와 배정도(double-precision, 64-bit)를 표현한다. 각각의 형은 한 개의 sign bit과 지수를 지수를 표현하는 부분(exponent) 그리고 가수를 표현하는 부분(fraction)으로 구성된다.

[cols=">2,>1,>1,>1,<1",frame="topbot",options="header"]
.IEEE Std 744-1985 Single/Double Precesion
|====
...
10111111 11100110 00000000 00000000 00000000 00000000 00000000 00000000
 ...
 exponent = 1022
----
표현된 실수는 `(-1)^1^ * (1+0.375) * 2^(1022-1023)^ = -1 * 1.375 * 2^-1^ = -0.6875` 이다.

==== Denormalized values

앞서 살펴 본 정규화된(normalized) 형식에서는 `0.0` 을 표현할 수 없다. 아래와 같이 표현했다고 할지라도 암묵적으로 가수(fraction) 부분에 더해지는 값(`1.0`) 때문에
----
S = 0     F = 0.0
-         ------- -------- --------
00111111 10000000 00000000 00000000
 ...
 exponent = 127
----
실제로 표현된 값은 `(-1)^0^ * (1+0.0) * 2^(127-127)^ = 1 * 1.0 * 2^0^ = 1.0` 이다.

이를 해결하기 위해 exponent를 구성하는 비트들이 모두 `0` 일 경우 비정규화된(denormalized) 값으로 구분되며 `I` 의 값으로 `0.0` 을 사용하고 `E = 1 - bias` 이다.

===== &#177;Zero
흥미롭게도 sign bit 에 따라서 `+0.0` 과 `-0.0` 을 따로 표현할 수 있다. 다음의 값을 실수로 계산해 보면
----
s = ?     fraction = 0....0
-         ------- -------- --------
...
===== Not a Number
exonent 부분을 구성하는 비트들이 모두 `1` 이고 fraction 부분의 비트들중 하나라도 `1` 이면 논리적으로 표현할 수 없는 수를 나타낸다. (fraction 비트들이 모두 `0` 이면 Infinity이다.)
...
==== Float#toIntBits(float) / Flaot#toRawIntBits(float)
