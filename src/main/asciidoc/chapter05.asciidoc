...
****
자바의 `int` 형은 몇 비트인가요? `signed` 형 인가요? 아니면 `unsigned` 형 인가요? 표현할 수 있는 최소값과 최대값은 무엇인가요?
****
...
- 정수형(integral types)에는 `byte`, `short`, `int`, `long`, 그리고 `char` 가 있다.
...
N개의 비트로 이루어져 있고 부호를 가지며 2의 보수체계를 사용하는 정수형으로 표현할 수 있는 값의 범위는 무엇일까? 아래 세개의 비트를 가지고 실험해 보자. `000` 부터 시계방향으로 1씩 더하면서 계산해 보자.
...
보는 바와 같이 `0b011 (+3)` 에서 `1` 을 더했을때 `0b100 (+4)` 이 되지만 첫번째 비트가 `1` 이므로 음수(`-4`)로 처리된다.

최대값은 첫번째 비트가 `0` 이고 나머지 모든 비트가 `1` 인 값이고 최소값은 첫번째 비트가 `1` 이고 나머지 비트가 모두 `0` 인 값이다. 이를 수식으로 표현하면 최대값은 `+2^n-1^ - 1` 이고 최소값은 `-2^n-1^` 이다.

****
한 개의 비트로 이루어져 있고 부호를 가지며 2의 보수 체계를 사용하는 정수형으로 표현할 수 있는 최대값과 최소값은 무엇인가? +
`Minimum` = `+2^1-1^ - 1` = +
`Maximum` = `-2^1-1^ - 0` =
****
...
자바의 `float` 형과 `double` 형은 https://standards.ieee.org/findstds/standard/754-1985.html[IEEE 754-1985] 에 정의된 단정도(single-precision, 32-bit)와 배정도(double-precision, 64-bit)를 표현한다. 각각의 형은 부호를 표현하는 한 개의 비트와 지수(exponent)를 표현하는 부분, 그리고 가수(fraction)를 표현하는 부분으로 구성된다.

[cols=">2,>1,>1,>1,<1",frame="topbot",options="header"]
.IEEE Std 744-1985 Single/Double Precision
|====
|                       |sign|exponent|fraction|notes
|32-bit single precision|`1` |`8`     |`23`    |bias = `127`
|64-bit double precision|`1` |`11`    |`52`    |bias = `1023`
|====

각각의 부분을 비트별로 표현하면 다음과 같다.
----
32-bit: seeeeeee efffffff ffffffff ffffffff
64-bit: seeeeeee eeeeffff ffffffff ffffffff ffffffff ffffffff ffffffff ffffffff
----

표현하는 실수 값은 다음과 같이 계산할 수 있다.

`N = (-1)^s^ × (I + F) × 2^E^`

==== Normalized values

정규화된(normalized) 형식에서는 `I = 1.0` 이고 `E = exponent - bias` 이다. 예를들어 다음과 같은 32-bit 값을 계산하면
----
s = 0     F = 0*(1/2) + 1*(1/4) = 0.25
-         ------- -------- --------
01000000 00100000 00000000 00000000
 ------- -
 exponent = 128
----
표현된 실수는 `(-1)^0^ * (1.0 + 0.25) * 2^(128-127)^ = 1 * 1.25 * 2^1^ = 2.5` 이다.

또 다른 예로 다음의 64-bit 실수를 계산해 보면

----
s = 1        F = 0*(1/2) + 1*(1/4) + 1*(1/8) = 0.375
-            ---- -------- -------- -------- -------- -------- --------
10111111 11100110 00000000 00000000 00000000 00000000 00000000 00000000
 ------- ----
 exponent = 1022
----
표현된 실수는 `(-1)^1^ * (1.0 + 0.375) * 2^(1022-1023)^ = -1 * 1.375 * 2^-1^ = -0.6875` 이다.

==== Denormalized values

앞서 살펴 본 정규화된(normalized) 형식에서는 `0.0` 을 포함하여 `-1.0` 보다 크고 `+1.0` 보다 작은 수들을 표현할 수 없다. 아래와 같이 표현했다고 할지라도, 암묵적으로 가수(fraction) 부분에 더해지는 값(`1.0`) 때문에,
----
S = 0     F = 0.0
-         ------- -------- --------
00111111 10000000 00000000 00000000
 ...
 exponent = 127
----
실제로 표현된 값은 `(-1)^0^ * (1.0 + 0.0) * 2^(127-127)^ = 1 * 1.0 * 2^0^ = 1.0` 이다.

이를 해결하기 위해 exponent를 구성하는 비트들이 모두 `0` 일 경우 비정규화된(denormalized) 값으로 구분하며 `I` 의 값으로 `0.0` 을 사용하고 `E` 의 값으로 `1 - bias` 를 사용한다.

===== &#177;Zero

exponent 부분의 비트들이 모두 `0` 이고 fraction 부분의 비트들도 모두 `0` 이면 `0.0` 을 나타낸다. 흥미롭게도 sign bit 의 값에 따라 `+0.0` 과 `-0.0` 을 따로 표현할 수 있다. 다음의 값을 실수로 계산해 보면
...
?0000000 00000000 00000000 00000000
 ...
 exponent = 0
----
표현하는 값은 `(-1)^s^ * (0.0 + 0.0) * 0^(1-127)^ = 0.0` 이고 `s` 가 `0` 이면 `+0.0`, `s` 가 `1` 이면 `-0.0` 이다.

==== Special values

exponent 부분을 구성하는 비트들이 모두 `1` 이면 특별한 수를 나타낸다.

===== &#177;Infinity(&#8734;)

exponent 부분을 구성하는 비트들이 모두 `1` 이고 fraction 부분의 비트들이 모두 `0` 이면 무한대의 수를 나타낸다. Zero와 마찬가지로 sign 비트에 따라서 양의 무한대(+&#8734;)와 음의 무한대(-&#8734;)를 표현할 수 있다.

----
s = ?     fraction = 0...0
-         ------- -------- --------
?1111111 10000000 00000000 00000000
 ------- -
 exponent = 1...1
----
...
Exponent 부분을 구성하는 비트들이 모두 `1` 이고 fraction 부분의 비트들중 하나라도 `1` 이면 표현할 수 없는 수(Not a Number)를 나타낸다. (fraction 비트들이 모두 `0` 이면 Infinity이다.)
...
다음은 {apidocs-javase}/java/lang/Float.html#NaN[`java.lang.Float#NaN`] 과 {apidocs-javase}/java/lang/Double.html#NaN[`java.lang.Double#NaN`] 을 이진수로 표기한 내용이다.
...
NaN 도 두가지 형태로 구분할 수 있다. fraction 부분의 첫번째 비트가 `0` 이면 예외상황을 나타내는 signaling NaN 이고 `1` 이면 quiet NaN 이다. (위에서 살펴본 `Float#NaN` 과 `Double#NaN` 은 quiet NaN이다.)

.signaling NaN
----
s = ?     fraction = 0...1...
-         _------ -------- --------
?1111111 10000000 00000000 00000001
?1111111 10000000 00000000 00000010
?1111111 10000000 00000000 00000011
?1111111 10...... ........ ........
?1111111 10111111 11111111 11111101
?1111111 10111111 11111111 11111110
?1111111 10111111 11111111 11111111
 ------- -
 exponent = 1...1
----
...
?1111111 11000000 00000000 00000001
?1111111 11000000 00000000 00000010
?1111111 11...... ........ ........
...
다음은 `&#177;.0f` 와 {apidocs-javase}/java/lang/Float.html[`java.lang.Float`] 클래스에 정의된 상수들을 2진법으로 출력한 내용이다. (보는 바와 같이 `java.lang.Float.NaN` 은 quiet NaN 이다.)

.Zeros and Constants defined in java.lang.Float
----
1 00000000 00000000000000000000000 -0.000000e+00 -.0f
0 00000000 00000000000000000000000 +0.000000e+00 +.0f
0 11111110 11111111111111111111111 +3.402823e+38 Float.MAX_VALUE
0 00000000 00000000000000000000001 +1.401298e-45 Float.MIN_VALUE
0 00000001 00000000000000000000000 +1.175494e-38 Float.MIN_NORMAL
1 11111111 00000000000000000000000 -Infinity     Float.NEGATIVE_INFINITY
0 11111111 00000000000000000000000 +Infinity     Float.POSITIVE_INFINITY
0 11111111 10000000000000000000000 NaN           Float.NaN
----

===== floatToIntBits(float) / floatToRawIntBits(float)

앞서 설명했듯이 NaN 에 해당하는 값은 하나가 아니다. (exponent 부분이 모두 `1` 이고 fraction 부분에 하나 이상의 `1` 이 포함되어 있으면 모두 NaN 이다.) {apidocs-javase}/java/lang/Float.html#floatToIntBits-float-[`Float#floatToIntBits(float)`] 메서드는 주어진 수가 NaN 일 경우 `Float#NaN`(`0x7fc00000`, `0b01111111_11000000_00000000_00000000`) 로 변환하고 {apidocs-javase}/java/lang/Float.html#floatToRawIntBits-float-[`Float#floaToRawIntBits(float)`] 메서드는 비트정보를 잃지 않고 그대로 변환한다.
...
System.out.printf("   floatToIntBits: 0x%08x%n", i1);
System.out.printf("floatToRawIntBits: 0x%08x%n", i2);
----
----
   floatToIntBits: 0x7fc00000
floatToRawIntBits: 0xff9be37c
----

<<<
=== hex-codec-api

(거의 모든) 컴퓨터들은 자료를 저장할 때, `octet` 으로 불리는, 여덟개의 비트로 구성된 최소 단위의 숫자들로 나누어 저장한다. `a` 라는 문자는 (`US-ASCII` 로 인코딩 되었을 때) `01100001` 로 저장되고 `홍` 이라는 문자는 (`UTF-8` 로 인코딩 되었을 때) `111011011 00110011 0001101` 로 저장된다. 아래 표에 여덟 개의 비트로 표현할 수 있는 몇몇 값들을 10진법, 8진법, 2진법, 그리고 자바의 `byte` 값으로 표현하였다.

[cols=">1,>1,>1,>1,<2",frame="topbot",options="header"]
.some 8-bit values
|====
...
거의 모든 문자셋의 기본이 되는 `US-ASCII` 문자셋에는 줄바뀜(CR, LF), 탭(TAB) 등의 제어 문자들과 보통의 자판에서 찾아볼 수 있는 문자들을 포함하여 `128`(`2^7^`) 개의 코드가 정의되어 있다. 때문에 여덟 개의 비트로 표현할 수 있는 `256`(`2^8^`) 개의 수에 고유의 문자를 할당하는 것이 불가능하다. 이를 해결하는 방법 중에 하나가 `base16`(`hex`) 로 표현하는 것이다. `base16` 표기는 여덟 개의 비트를 네 개씩 두 그룹으로 나눈 후에 각 그룹의 값을 미리 정의된 문자로 변환한다.

우선 아래와 같이 여덟 개의 비트를 네 개씩 두 그룹으로 나눈다.

[source,text]
----
index  7 6 5 4 3 2 1 0    -> 0 ~ 255
upper  H H H H            -> 0 ~ 15
lower          L L L L    -> 0 ~ 15
----

그리고 (네 개의 비트로 구성되는) 각 그룹의 값(`0` ~ `15`)을 아래 표에 정의된 열여섯 개의 문자들 중 하나로 변환한다. (한 개의 숫자가 두개의 문자로 변환된다.)
...
일곱번 째 공백문자(`'{nbsp}'`)의 ascii 값은 `32`(`00100000`)이다. 이를 각 네 자리씩으로 나눈 `0010` 과 `0000` 에 대해 첫 번째 그룹의 값(`2`)에 해당하는 문자 `'2'` 와 두 번째 그룹의 값(`0`)에 해당하는 문자 `'0'` 이 결과값이다.
[source,text]
----
' ' = 0010 0000  ( 32, 0x20)
...
public interface HexEncoder {
    ...
    // 버퍼(encoded)에 담을 수 있는 공간(remaining)이 2 보다 작으면
    // BufferOverflowException 을 던진다.
...
----
    // decoded 버퍼에 남아있는(remaining) 각각의 바이트들을 변환하고
    // 그 결과를 encoded 버퍼에 담는다.
    // decoded 버퍼에서 소비된(변환된) 바이트의 개수를 반환한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=40..47]
----
...
<3> `decoded` 버퍼의 현재 위치(`position`)값과 미리 기록해 둔 값의 차이를 반환한다.
...
----
    // encoded 버퍼의 모든 바이트들을 변환하고 결과를 새 버퍼에 담아 반환한다.
    // 반환되는 버퍼는 바로 읽을 수 있는 상태이다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=56..61]
----
...
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexEncoder.java[lines=71..79]
----
...
`Base 16 Encoding` 으로 변환된 문자들을 다시 숫자들로 변환하기 위한 인터페이스이다.
...
public interface HexDecoder {
    ...
    // 버퍼에서 hex 문자 두개를 변환하여 한 개의 숫자(0 ~ 255)를 반환한다.
    // 버퍼에서 읽을 수 있는 바이트들의 개수가 두 개 이하면
    // BufferUnderflowException 을 던진다.
...
두 개의 버퍼를 전달받아 첫번째 버퍼의 문자들을 디코드한 후 두번째 버버에 담는다.

[source,java]
.HexDecoder.java#decode(ByteBuffer, ByteBuffer)
----
    // 입력 버퍼(encoded)에서 문자들을 변환하여 출력 버퍼(decoded)에 담는다.
    // 출력 버퍼에 추가된 바이트의 개수를 반환한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=40..47]
----
...
전달받은 버퍼의 문자를을 디코드한 후 그 결과를 새로운 버퍼에 담에서 반환한다.

[source,java]
.HexDecoder.java#decode(ByteBuffer)
----
    // 주어진 버퍼(encoded)에 남아있는(remaining) 문자들을 디코드한 후 그 결과를 새 버퍼에 담아 반환한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=56..61]
----
...
----
    // 주어진 문자열(encoded)를 변환하고 그 결과를 새로운 문자열로 반환한다.
    // 변환된 바이트들과 명시된 문자셋(charset)을 이용하여 문자열을 생성한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/HexDecoder.java[lines=71..79]
----
...
기록되는 각각의 바이트들 각각을 자동으로 (두 개의) `base16` 문자들로 바꾸어 기록하는 `OutputStream` 이다. {apidocs-javase}/java/io/FilterOutputStream.html[`java.io.FilterInputStream`] 클래스를 확장하여 작성되었다.
...
public class HexOutputStream extends FilterOutputStream {
    ...
    protected HexEncoder enc; // 인코더 개체
...
[TIP]
====
{apidocs-javase}/java/io/FilterOutputStream.html[`FilterOutputStream`] 클래스의 생성자는 `out` 인자가 `null` 인지 확인하지 않는다. 대신 `protected` 접근자를 가지는 {apidocs-javase}/java/io/FilterOutputStream.html#out[`out`] 필드를 통해 값을 수정할 수 있다. 아마도 `null` 을 이용하여 생성한 후 실제로 사용할 필요가 있을 때 게으르게(lazily) 초기화될(initialized) 수 있는 여지를 남겨둔 듯 하다. `HexOutputStream` 클래스의 생성자도 `enc` 인자가 `null` 인지 확인하지 않는다. `null` 값을 이용하여 생성한 후 다음과 같이 사용할 수도 있다.
[source,java]
----
final OutputStream hos = new HexOutputStream(null, null) {
    @Override
    public void write(final byte b) throws IOException {
        if (out == null) out = open(); // lazily
        if (enc == null) enc = load(); // lazily
        super.write(b);
    }
};
----
...
<3> `buf` 를 뒤집어 변환된 바이트들을 기록할 준비를 한다.
<4> 변환된 두개의 문자들을 `out` 에 기록한다.
<5> 다음 변환작업을 위해 `buf` 를 비운다.
...
이제 `HexOutputStream` 클래스와 `HexInputStream` 클래스가 수행하는 기능을 `java.nio.channels` 패키지에 정의된 클래스들을 사용하여 구현해 보자. `FilterOutputStream` 과 `FilterInputStream` 에 해당하는 클래스들이 존재하지 않으므로 `WritableByteChannel` 인터페이스와 `ReadableByteChanel` 인터페이스의 공통 부모인 `java.nio.channels.Channel` 인터페이스를 구현하는 클래스를 하나 정의하고 공통의 기능을 미리 구현하도록 하겠다. `Filter(Input|Output)Stream` 과는 다르게 `Supplier` 를 이용한다.

[source,java]
.FilterChannel.java
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=30..30]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=37..41]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=80..95]
----
<1> 생성자를 통해 전달받은 `Supplier<T>`.
<2> `isOpen` 메서드를 구현하기 위한 변수.
<3> `channelSupplier` 를 통해 (게으르게) 제공된 채널.

===== isOpen()

채널이 열려 있는지 확인한다. `FilterChannel#isOpen()` 메서드는 `closed` 변수의 값이 `false` 인지를 반환한다.

[source,java]
.FilterChannel.java#isOpen()
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=50..53]
----

===== close()

채널을 닫는다. `FilterChannel#close()` 메서드는 `channel` 이 `null` 이 아닐 경우 `channel` 의 `close()` 메서드를 호출하고 `closed` 필드의 값을 `true` 로 변환한다.

[source,java]
.FilterChannel.java#close()
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=62..72]
----

===== channel()

생성자를 통해 전달된 `channelSupplier` 로부터 게으르게 생성된 `channel` 개체를 반환한다.

[source,java]
.FilterChannel.java#channel()
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/FilterChannel.java[lines=80..88]
----
<1> 이미 닫혔으면 예외를 던진다.
<2> 게으르게 생성한다.
...
.WritableFilterChannel.java
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableFilterChannel.java[lines=28..30]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableFilterChannel.java[lines=37..39]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableFilterChannel.java[lines=48..53]
----
...
.ReadableFilterChannel.java
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableFilterChannel.java[lines=28..30]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableFilterChannel.java[lines=37..39]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableFilterChannel.java[lines=49..53]
----
...
.WritableHexChannel.java
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=37..38]

    // 인코더 객체도 Supplier 를 통해 게으르게 생성한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=51..56]

    // 게으르게 생성된 encoder 를 반환한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=86..94]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=97..100]
----
...
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/WritableHexChannel.java[lines=66..77]
----
...
.ReadableHexChannel.java
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=37..38]

    // 인코더 객체도 Supplier 를 통해 게으르게 생성한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=53..58]

    // 게으르게 생성된 encoder 를 반환한다.
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=100..108]

include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=111..114]
----
...
----
include::codes/hex-codec-api/src/main/java/com/github/jinahya/verbose/hex/ReadableHexChannel.java[lines=72..91]
----
...
. `WritbleHexChannel` 클래스의 `write(ByteBuffer)` 메서드는 매번 변환작업에 필요한 중간 단계의 버퍼(`aux`) 를 생성한다. 해당 버퍼를 개체 변수로 변경하라.
+
[source,java]
.WriableHexChannel.java
----
    @Override
    public int write(final ByteBuffer src) throws IOException {
        ...
        if (src.capacity() == 0 || src.remaining() == 0){
            return 0;
        }
        if (aux == null) {
            // 매번 동일한 src 가 사용된다고 가정한다.
            aux = allocate(src.capacity() << 1);
        }
...
. `ReadableHexChannel.java` 에 대하여 위 항목에 해당하는 내용을 수행하라.
...
[source,java]
----
        switch (decoded & 0b1111) { // 하위 네 개의 비트만을 기준으로 동작한다.
            // ...
        }
----
====
...
NOTE: 앞서 `HexEncoderImpl` 클래스는 무조건 대문자로 변환했지만 여기서는 소문자로 변환된 값들도 모두 처리한다.

[CAUTION]
====
역시나 가정에 기반하여 처리한 부분이 있다. 다음과 같이 모든 경우에 대해 확실한 코드를 사용하는 편이 더 훌륭한 코드가 아닐까 한다.
[source,java]
----
            case 'a': //  97, 0x61
            case 'b': //  98, 0x62
...
----
<1> 첫 번째 문자를 디코드하고 윈쪽으로 `4` 만큼 옮긴 값에 두번째 문자를 디코드한 값을 OR 연산하여 반환한다.
...
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/Rfc4648TestVectors.java[lines=37..45]
...
다음과 같이 메서드를 하나 추가하여 데이터를 사용할 수 있게 했다.

[source,java]
.Rfc4648TestVectors.java#base16(BiConsumer)
----
    // 주어진 BiConsumer 의 accept(String, String) 메서드에 BASE16 리스트의 값들을 전달한다.
    // 첫 번째 인자에는 BASE16 리스트의 짝수 번째 문자열(decoded)을,
    // 두 번째 인자에는 BASE16 리스트의 홀수 번째 문자열(encoded)을 제공한다.
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/Rfc4648TestVectors.java[lines=102..106]
----
...
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=43..48]
----
<1> 테스트 벡터에 정의된 값(`d`, 인코드할 값)을 변환한 후 제시된 값(`e`, 인코드된 결과값)과 비교한다.

===== encodeVerboseEncodeCommons()

`HexEncoderImpl` 클래스의 결과값을 {apidocs-commons-codec}/org/apache/commons/codec/binary/Hex.html#encode(byte%5B%5D)[`encode([B)[B`] 메서드의 결과값과 비교한다.
...
[source,java]
.HexEncoderImpl.java#encodeVerboseEncodeCommons()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=54..72]
----
<1> 무작위의 데이터를 생성한다.
<2> `Hex` 클래스의 `encode([B)[B` 메서드를 이용하여 변환한다. (해당 메서드는 `10 ~ 15` 의 값을 소문자로 변환하므로 적절한 사후 처리 작업을 추가하였다.)
<3> `HexEncoderImpl` 클래스를 이용하여 변환한다.
<4> 변환된 두 값을 비교한다 .

===== encodeVerboseDecodeCommons()

아래는 `HexEncoderImpl` 클래스를 이용하여 인코드한 값을 `Hex` 클래스를 이용하여 디코드한 후 결과값을 비교한다.
...
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=79..87]
----
<1> 인코드할 배열을 생성한다.
...
===== encodeVerboseEncodeGuava()

`HexEncoderImpl` 클래스의 결과값을 {apidocs-guava}/com/google/common/io/BaseEncoding.html#encode(byte%5D%5B)[`encode([B)Ljava.lang.String;`] 메서드의 결과값과 비교한다.

[ditaa]
....
              +--------------+
  created --> |{io}          |
              | BaseEncoding | --> encoded1
              |cGRE          |        |
              +--------------+        |
                                   compare
              +--------------+        |
...
[source,java]
.HexEncoderImpl.java#encodeVerboseEncodeGuava()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=92..109]
----
<1> 무작위의 데이터를 생성한다.
<2> `BaseEncoding` 클래스의 `encode([B)Ljava.lang.String;` 메서드를 이용하여 변환한다. (해당 메서드는 `String` 형을 반환하므로 적절한 사후 처리 작업을 추가하였다.)
<3> `HexEncoderImpl` 클래스를 이용하여 변환한다.
<4> 변환된 두 값을 비교한다 .

===== encodeVerboseDecodeGuava()

`HexEncoderImpl` 클래스로 인코드한 후 `BaseEncoding` 클래스로 디코드한 후 결과값을 비교한다.
...
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexEncoderImplTest.java[lines=114..131]
----
<1> 무작위의 데이터를 생성한다.
<2> 생성된 데이터를 `HexEncoderImpl` 클래스를 이용하여 인코드한다.
<3> 인코드된 값을 `BaseEncoding` 클래스를 이용하여 디코드한다.
<4> 디코드된 값을 생성한 값과 비교한다.
...
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexDecoderImplTest.java[lines=41..46]
----
...
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexDecoderImplTest.java[lines=51..68]
----
<1> 무작위의 데이터를 생성한다.
<2> 생성된 데이터럴 `Hex` 클래스를 이용하여 인코드한다.
<3> 인코드된 데이터를 `HexDecoderImpl` 클래스를 이용하여 디코드한다.
<4> 디코드된 데이터를 생성된 디에터와 비교한다.

===== encodeGuavaDecodeVerbose()

`BaseEncoding` 클래스를 이용하여 `HexDecoderImpl` 클래스의 기능을 확인한다.
...
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexDecoderImplTest.java[lines=73..90]
----
<1> 무작위의 데이터를 생성한다.
<2> 생성된 데이터럴 `BaseEncoding` 클래스를 이용하여 인코드한다.
<3> 인코드된 데이터를 `HexDecoderImpl` 클래스를 이용하여 디코드한다.
<4> 디코드된 데이터를 생성된 디에터와 비교한다.
...
무작위로 값을 생성한 후 `HexInputStream` 과 `HexOutputStream` 클래스를 이용하여 인코드/디코드 한 후 값을 비교한다.
...
[source,java]
.HexStreamTest.java#encodedDecodeStream()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=38..39]
        // @todo: implement.
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=69..69]
----
...
.HexStreamTest.java#encodedDecodeStream()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=40..44]
----

생성된 데이터를 `HexOutputStream` 클래스를 이용하여 인코드한다.

[source,java]
.HexStreamTest.java#encodedDecodeStream()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=45..56]
----

인코드된 데이터를 `HexInputStream` 클래스을 이용하여 디코드한다.

[source,java]
.HexStreamTest.java#encodedDecodeStream()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=57..67]
----

디코드된 값을 최초에 생성한 데이터와 비교한다.

[source,java]
.HexStreamTest.java#encodedDecodeStream()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=68..68]
----

===== encodeDecodeFile()

무작위로 생성된 데이터를 가지는 파일을 생성한 후 변환과정을 거친 값과 비교한다.

[ditaa]
....
                +-----------------------------------------+
  created ----> |{io}                                     |
                |FileInputStream -(copy)-> HexOutputStream|
     ^          |cBLU                                     | -----\
     |          +-----------------------------------------+      |
 ...
 (compare)                                                    encoded
     ...
     |          +-----------------------------------------+      |
     v          |{io}                                     | <----/
                |FileOutputStream <-(copy)- HexInputStream|
  decoded <---- |cGRE                                     |
                +-----------------------------------------+
....
...
.HexStreamTest.java#encodeDecodeFile()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=80..90]
----
...
.HexStreamTest.java#encodeDecodeFile()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=91..103]
----

인코드된 파일을 또 다른 파일(`decoded`)에 `HexInputStream` 클래스를 이용하여 복사한다. (디코드한다.)

[source,java]
.HexStreamTest.java#encodeDecodeFile()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=104..116]
----
...
.HexStreamTest.java#encodeDecodeFile()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=117..118]
----
...
.HexStreamTest.java#encodeDecodeFile()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexStreamTest.java[lines=119..124]
----

==== HexChannelTest.java

`WritableHexChannel` 클래스와 `ReadableHexChannel` 클래스를 이용하여 `HexEncoderImpl` 클래스와 `HexDecoderImpl` 클래스의 기능을 확인하는 클래스이다.
...
무작위 데이터를 생성한 후 (`HexEncoderImpl` 개체를 사용하는) `WritableHexChannel` 개체를 이용하여 인코드한 후 (`HexDecoderImpl` 개체를 사용하는) `ReadableHexChannel` 개체를 이용하여, 다시, 디코드한 값을 비교한다.

[ditaa, "HexChannelTest_encodeDecodeChannel"]
....
                +------------------+
  created ----> |{io}              |
                |WritableHexChannel|
...
기본적으로 `HexStreamTest#encodeDecodeStream()` 메서드와 동일한 접근방식을 취한다.

무작위로 데이터를 생성한다.

[source,java]
.HexChannelTest.java#encodeDecodeChannel()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=61..64]
----

`HexEncoderImpl` 클래스와 `WritableHexChannel` 클래스를 이용하여 인코드한다.

[source,java]
.HexChannelTest.java#encodeDecodeChannel()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=65..77]
----

`HexDecoderImpl` 클래스와 `ReadableHexChannel` 클래스를 이용하여 디코드한다.

[source,java]
.HexChannelTest.java#encodeDecodeChannel()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=78..90]
----

최초 생성한 값과 변환작업을 거친 값을 비교한다.

[source,java]
.HexChannelTest.java#encodeDecodeChannel()
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=91..91]
----

===== encodeDecodePath(String)

`java.nio.file.Path` 클래스를 이용하여 `HexEncoderImpl` 클래스와 `HexDecoderImpl` 클래스의 기능을 확인하는 메서드이다.

[ditaa, "HexChannelTest_encodeDecodePath"]
....
                +---------------------------------+
  created ----> |{io}                             |
...
.HexChannelTest.java#encodeDecodePath(String)
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=103..114]
----
...
.HexChannelTest.java#encodeDecodePath(String)
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=115..129]
----
...
.HexChannelTest.java#encodeDecodePath(String)
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=130..144]
----
...
.HexChannelTest.java#encodeDecodePath(String)
----
include::codes/hex-codec-lib/src/test/java/com/github/jinahya/verbose/hex/HexChannelTest.java[lines=145..145]
----
...
`a/b?c` 에서 `/` 와 `?` 가 각각 `%2F` 와 `%3F` 변환되었으며 `e&f=g/h` 에서 `&`, `=`, `/` 가 `%26`, `%3D`, `%2F` 로 변환되었다.
...
아래는 `hello, world` 문자열이 percent encoding 된 모양을 나타낸다.
...
0x 68 65 6C 6C 6F 2C 20 77 6F 72 6C 64
   --             -- --
    |             -- -- --
   --                      -- -- --
    h  e  l  l  o  %  2  C  %  2  0  w  o  r  l  d
   -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
0x 68 65 6C 6C 6F 25 32 63 25 32 30 77 6F 72 6C 64
----

보는 봐와 같이 `,` 와 `'{nbsp}'` 가 각각 `%2C` 와 `%20` 으로 변환되었다.
...
public interface PercentEncoder {
    ...
    // 숫자(decoded) 하나를 인코딩 한 후 그 결과(한 개 혹은 세 개의 문자)를 버퍼(encoded)에 담는다.
    // 버퍼(encoded)에 공간이 충분치 않으면 BufferOverflowException 을 던진다.
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/PercentEncoder.java[lines=26..26]
}
----
...
두 개의 버퍼를 전달받아 첫 번째 버퍼의 바이트들(숫자들)을 (문자들로) 변환한 후 두 번째 버퍼에 담는다. `HexEncoder` 인터페이스와는 다르게 변환할 바이트가 몇 개의 문자로 변환될 지 알 수 없다. (알 수는 있지만 미리 알 필요가 없다고 상정한다.) 때문에 `encodeOcetet(int, ByteBuffer)` 메서드가 `BufferOverflowException` 을 던지는지 확인한다.
[source,java]
.PercentEncoder.java#encode(ByteBuffer, ByteBuffer)
----
    // decoded 버퍼의 바이트들을 인코드한 후 encoded 버퍼에 담는다.
    // decoded 버퍼에서 소비된 바이트 개수를 반환한다.
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/PercentEncoder.java[lines=37..50]
----
...
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/PercentEncoder.java[lines=59..63]
----
<1> `decoded` 버퍼에 가용한 바이트 개수의 세 배에 해당하는 버퍼를 생성한다. (모든 바이트들이 `%XX` 형태로 변환된다고 가정한다.)
<2> `encode(ByteBuffer, ByteBuffer)` 메서드를 호출해 변환한다.
<3> `encoded` 버퍼를 뒤집어(flip) 읽을 수 있는 상태로 만든 후 반환한다.

===== encode(String, Charset)

주어진 문자열을 인코드한 후 반환한다.

[source,java]
.PercentEncoder.java#encode(String,Charset)
----
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/PercentEncoder.java[lines=74..81]
----
...
----
    // encoded 버퍼의 문자들을 디코드한 숫자들을 decoded 버퍼에 담는다.
    // decoded 버퍼에 추가된 숫자들의 개수를 반환한다.
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/PercentDecoder.java[lines=35..49]
----
...
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/PercentDecoder.java[lines=58..63]
----
<1> `encoded` 버퍼와 같은 용량의 버퍼를 생성한다. (모든 바이트들이 `%XX` 형태가 아닌 그대로 변환되었다고 가정한다.)
<2> `decode(ByteBuffer, ByteBuffer)` 메서드를 이용해 디코드한다.
<3> 반환할 버퍼(`decoded`)를 뒤집어(`flip()`) 읽을 수 있는 상태로 만든다.

===== decode(String, Charset)

주어진 문자열을 디코드하고 그 결과값과 주어진 문자셋을 이용하여 생성된 문자열을 반환한다.
...
----
<1> 문자열로부터 입력 배열을 생성한다.
<2> 입력 배열과 같은 크기를 가지는 출력 배열을 생성한다. (모든 숫자들이 `%XX` 형태가 아닌 그대로 변환되었다고 가정한다.)
<3> 두 배열을 감싸는(wrap) 버퍼들을 생성한 후 `decode(ByteBuffer, ByteBuffer)` 메서드을 이용해 변환한다.
<4> 변환된 출력 배열로부터 `charset` 을 이용해 출력 문자열을 생성한다.

==== Exercises

. `WritablePercentChannel` 클래스를 완성하라.
+
[source,java]
.ReadablePercentChannel.java#write(ByteBuffer)
----
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/WritablePercentChannel.java[lines=70..80]
----

. `ReadablePercentChannel` 클래스를 완성하라.
+
[source,java]
.ReadablePercentChannel.java#read(ByteBuffer)
----
include::codes/percent-codec-api/src/main/java/com/github/jinahya/verbose/percent/ReadablePercentChannel.java[lines=70..80]
----
...
`PercentEncoder` 인터페이스를 구현한 클래스이다.
...
===== PercentEncoder(Supplier<HexEncoder>)

`HexEncoder` 개체를 제공하는 `Supplier` 를 받는 생성자이다.

[source,java]
.PercentEncoderImpl.java#PerentEncoder(Supplier<HexEncoder>)
----

include::codes/percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentEncoderImpl.java[lines=38..42]

include::codes/percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentEncoderImpl.java[lines=74..83]
----
...
`HexEncoder` 개체를 `ServiceLoader` 를 이용해서 적재하는 생성자이다.
...
----
<1> `ServiceLoader#load(Class)` 메서드를 통해 객체를 적재하고 제공하는 `Supplier` 이다.
...
----
include::codes/percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentEncoderImpl.java[lines=53..67]
----
<1> 변환할 필요가 없는 숫자는 그대로 버퍼에 담는다.
<2> 변환이 필요한 문자는 한 개의 `"%"` 를 버퍼에 담고
<3> `HexEncoder` 인터페이스의 `encodeOctet(int, ByteBuffer)` 메서드를 이용해 두개의 hex 문자로 변환하여 버퍼에 담는다.

==== PercentDecoderImpl.java

`PercentDecoder` 인터페이스를 구현한 클래스이다.
...
===== PercentDecoderImpl(Supplier<HexDecoder>)

`HexDecoder` 개체를 제공하는 `Supplier` 개체를 받는 생성자이다.

[source,java]
.PercentEncoderImpl.java
----
include::codes/percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentDecoderImpl.java[lines=37..41]

include::codes/percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentDecoderImpl.java[lines=62..71]
----
<1> `HexDecoder` 개체를 제공하는 `Supplier` 개체이다.
<2> `decoderSupplier` 를 통해 게으르게 얻을 `HexDecoder` 이다.

===== PercentDecoderImpl()

`ServiceLoader` 클래스를 이용하는 `Supplier` 를 사용하는 생성자이다.

[source,java]
.PercentEncoderImpl.java#PercentEncoderImpl()
----
include::codes/percent-codec-lib/src/main/java/com/github/jinahya/verbose/percent/PercentDecoderImpl.java[lines=49..51]
----

===== decodeOctet(int, ByteBuffer)
...
----
<1> 문자를 하나 읽는다.
<2> 첫 문자가 `%` 이면 `HexDecoder` 인터페이스의 `decodeOctet(ByteBuffer)` 메서드를 이용하여 디코드한 값을 반환한다.
