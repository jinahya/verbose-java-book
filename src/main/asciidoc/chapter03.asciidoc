...
[source,text]
----
       0     offset                  offset + 12     array.length - 1
            ↙                       ↙               ↙
       - - - - - - - - - - - - - - - - - - - - - - -
...
[NOTE]
====
해당 `for` 순환문 는 다음과 같은 코드에, 설명을 위해, 줄바꿈이 추기된 것이다.

[source,java]
----
    for (buffer.flip(); buffer.hasRemaining(); channel.write(buffer)) {
        // empty
    }
----
====

NOTE: `for` 순환문이 동작하는 방식을, 다른 이에게, 설명할 수 있는지 스스로 확인해보자.
