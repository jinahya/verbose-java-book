...
문자열 하나를 표준 입출력을 이용하여 화면에 인쇄하는 방법은 이미 알고 있다.
...
자바 소스코드를 컴파일하면 `.class` 파일이 생성되는데 그 파일의 첫 네 개의 바이트들은 해당 파일이 자바 클래스파일임을 명시하는 특별한 값이다.
...
$ xxd -l 4 target/classes/com/github/jinahya/verbose/hello/SuccinctHelloWorld.class
00000000: cafe babe
...
===== set(byte[], int)
우리가 구현해 볼 메서드이다. 적절한 인자가 제공되었을 때 아래와 같이 값이 배열에 값이 저장된다.
...
다음은 배열의 크기가 충분하지 않을 경우에 대한 시험항목이다.
...
[source,java]
----
include::codes/hello-world-lib/src/test/java-guice/com/github/jinahya/verbose/hello/HelloWorldGuiceModule.java[lines=36..38]
----
...
[source,java]
----
include::codes/hello-world-lib/src/test/java-guice/com/github/jinahya/verbose/hello/HelloWorldGuiceModule.java[lines=39..43]
----
...
[source,java]
----
include::codes/hello-world-lib/src/test/java-guice/com/github/jinahya/verbose/hello/HelloWorldGuiceModule.java[lines=45..50]
----
...
hello-world-app 모듈은 hello-world-api 모듈에 정의된 클래스들만을 사용하고 hello-world-lib 모듈에 정의된 클래스들은 실제로 실행될 때만 사용되로록 설정하였다. 아래는 hello-world-app 모듈의 pom.xml 파일에 정의된 의존성들을 보여준다.

[source,xml]
----
include::codes/hello-world-app/pom.xml[lines=31..42]
----

==== HelloWorldMain.java

우리가 작성한 `HelloWorldImpl` 클래스를 이용하여 `hello, world` 를 `System.out` 를 이용해서 화면에 출력하는 프로그램이다. 앞서 구현했던 서비스 확장 기능을 사용한다.

[source,java]
.HelloWorldMain.java
----
public class HelloWorldMain {
include::codes/hello-world-app/src/main/java/com/github/jinahya/verbose/hello/HelloWorldMain.java[lines=28..32]
}
----
<1> `HelloWorld` 객체를 적재한다.
<2> 적재된 `HelloWorld` 객체의 `write(OutputStream)` 메서드에 `System.out` 을 전달한다.
<3> 반환된 값(`System.out`)에 `println()` 을 호출해서 줄바꿈을 추가한다.
