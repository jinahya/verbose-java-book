...
include::codes/hello-world-api/src/main/java/com/github/jinahya/verbose/hello/HelloWorld.java[lines=67..83]
----
...
<5> 배열에 채워진 값들을 버퍼로 옮긴다. (버퍼의 `position` 값이 배열 길이만큼 증가한다.)
...
include::codes/hello-world-api/src/main/java/com/github/jinahya/verbose/hello/HelloWorld.java[lines=95..105]
----
<1> `stream` 인자가 `null` 이면 `NullPointerException` 을 던진다.
<2> 배열을 생성하고 `set(byte[], int)` 메서드를 이용해 값을 채운다.
<3> {apidocs-javase}/api/java/io/OutputStream.html#write-byte:A-[`OutputStream#write(byte[\])`] 메서드를 이용해 스트림에 기록한다.
...
include::codes/hello-world-api/src/main/java/com/github/jinahya/verbose/hello/HelloWorld.java[lines=121..132]
----
<1> `channel` 인자가 `null` 이면 `NullPointerException` 을 던진다.
<2> 버퍼를 생성하고 `put(ByteBuffer)` 메서드를 이용해 값을 채운다.
<3> 버퍼를 읽기 상태로 전환한 후,
<4> 버퍼에 값이 남아있는 동한 `channel` 에 기록한다.

NOTE: {apidocs-javase}/api/java/nio/channels/WritableByteChannel.html[`WritableByteChannel`] 인터페이스를 구현한 클래스들 중에는 내부적인 상태 혹은 설정에 따라 http://docs.oracle.com/javase/8/docs/api/java/nio/channels/WritableByteChannel.html#write-java.nio.ByteBuffer-[`write(ByteBuffer)`] 메서드에서 버퍼 모든 바이트들을 한번에 기록하지 않을 수도 있다.
...
----
include::codes/hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=57..61]
----
<1> 아무런 동작도 하지 않는다.
...
[source,java]
.HelloWorldTest.java#testPutBuffer()
----
include::codes/hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=66..82]
----
<1> `buffer` 값이 `null` 이면 `NullPointerException` 을 던지는지 확인한다.
<2> `HelloWorld#BYTES(12)` 보다 작은 크기의 `buffer` 를 전달했을 때  `BufferOverflowException` 을 던지는지 확인한다.
<3> `byte[]` 를 사용하는 `buffer` 를 생성한 후 `put` 메서드를 호출한 후 전달한 버퍼를 반환하는지 확인한다.
<4> `byte[]` 를 사용하지 않는 버퍼를 생성한 후 `put` 메서드를 호출한 후 전달한 버퍼를 반환하는지 확인한다.
...
----
include::codes/hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=89..96]
----
...
----
include::codes/hello-world-api/src/test/java/com/github/jinahya/verbose/hello/HelloWorldTest.java[lines=103..111]
----
...
----
<1> 해당 항목에 위해된지 않는 `offset` 값을 사용한다.
...
----
    // offset 이 0 보다 작으면 IndexOutOfBoundsException 을 던져야 한다.
...
다음은 배열에 공간이 충분치 않을 경우에 대한 시험항목이다.
