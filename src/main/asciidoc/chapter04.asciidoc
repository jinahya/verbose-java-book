...
[TIP]
====
`OutputStream` 객체 혹은 `Writer` 객체에 값을 기록한 후 `close()` 메스드를 호출하기 전에 (혹은 필요한 때마다) `flush()` 메서드를 꼭 호출해서 (아직 파일에 기록도지 않은 채) 쌓여있는 데이터를 밀어내야 한다.
====
...
****
`copy1(ReadableByteChannel, WritableByteChannel, ByteBuffer)` 메서드는 `write(ByteBuffer)` 메서드를 한 번만 호출하는 대신 `compact()` 메서드를 사용해서 데이터를 계속 버퍼의 앞쪽으로 모으는 작업을 한다. [참조: http://shop.oreilly.com/product/9780596002886.do[Java NIO, O'Reilly Media]]
****
...
****
`copy2(ReadableByteChannel, WritableByteChannel, ByteBuffer)` 메서드는 데이터를 앞쪽으로 모으는 작업(`compact()`)을 하지 않는 대신 버퍼에 데이터가 남아 있을 동안 `write(ByteBuffer)` 메서드를 계속 호출한다. [참조: http://shop.oreilly.com/product/9780596002886.do[Java NIO, O'Reilly Media]]
****
...
----
include::codes/verbose-java-utils/src/main/java/com/github/jinahya/verbose/util/IoUtils2.java[lines=98..108]
----
...
----
include::codes/verbose-java-utils/src/main/java/com/github/jinahya/verbose/util/IoUtils2.java[lines=110..124]
----
...
.IoUtils2.java#copy2(Path, Path);
----
include::codes/verbose-java-utils/src/main/java/com/github/jinahya/verbose/util/IoUtils2.java[lines=126..140]
----
...
`HelloWorld` 인터페이스를 구현한 어떤 클래스가 주어졌을 때 각 메서드에서 로그를 남기려면 어떻게 해야 할까? 다음과 같이 그 클래스를 확장하여 로그를 남기는 코드를 삽입하면 된다.
...
----
<1> 아무런 동작도 하지 않는 `HelloWorld` 객체를 생성한다.
<2> proxy 객체를 하나 생성한다.
<3> proxy 객체에 특정 메서드를 호출한다.
...
----
    public long copy1(final ReadableByteChannel redable, final WritableByteChannel writable,
        ...
        // ...
include::codes/verbose-java-utils/src/main/java/com/github/jinahya/verbose/util/IoUtils2.java[lines=52..52]
        // ...
    }
----

. 위 항목과 동인할 문제점이 있는 코드를 모두 수정하라.
...
public static <T extends WritableByteChannel> T nonBlocking(
    ...
    // channel.isBlocking() 메서드가 true 를 반환하면
    // 해당 채널을 그대로 반환한다.
...
public static <T extends ReadableByteChannel> T nonBlocking(
    ...
    try {
        ...
        // channel is not an instance of AbstractSelectableChannel
    }
    // ...
)
----
